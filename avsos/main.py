...
def save_report(report_data, file_formats, output_file, encrypt_output=False, graph_file=None):
    ...
    if 'txt' in formats:
        with open(output_file_no_ext + '.txt', 'w') as f:
            ...
            if encrypt_output:
                encrypted_data, _ = encryption.encrypt_data(formatted_output)
    ...
    if args.subdomains_file:
        try:
            with open(args.subdomains_file, "r") as f:
                ...
                amass_output = subprocess.run(["amass", "enum", "-df", args.subdomains_file], check=True, text=True, capture_output=True)
    ...
    if args.nmap or args.all:
        ...
        if subdomains:
            ...
            try:
                ...
                if subdomain_nmap_report_str is not None:
